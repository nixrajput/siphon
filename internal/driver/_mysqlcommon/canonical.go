...
func (c *Conn) EmitCanonical(ctx context.Context, schema *canonical.CanonicalSchema, w io.Writer) error {
	...
	for _, t := range schema.Tables {
		if err := myEmitTable(ctx, c.db, c.engine, t, w); err != nil {
			return err
		}
	}
	return nil
...
func myEmitTable(ctx context.Context, db *sql.DB, engine string, t canonical.CanonicalTable, w io.Writer) error {
...
func (c *Conn) ConsumeCanonical(ctx context.Context, r io.Reader) error {
	...
	for _, t := range header.Schema.Tables {
		stmt, err := canonical.BuildCreateTableSQL(c.engine, t)
		if err != nil {
			return err
		}
	...
	for {
		...
		if err := myInsertRow(ctx, c.db, c.engine, row); err != nil {
			return err
		}
	}
	return nil
...
func myInsertRow(ctx context.Context, db *sql.DB, engine string, row canonical.CanonicalRow) error {
...
func (c *Conn) ApplyChange(ctx context.Context, ch canonical.CanonicalChange) error {
	...
	case canonical.OpInsert:
		...
		sort.Strings(cols)
		stmt, err := canonical.BuildIdempotentInsertSQL(c.engine, ch.Table, cols)
		if err != nil {
			return err
		}
	...
	case canonical.OpUpdate:
		...
		stmt, err := canonical.BuildUpdateSQL(c.engine, ch.Table, setCols, keyCols)
		if err != nil {
			return err
		}
	...
	case canonical.OpDelete:
		...
		stmt, err := canonical.BuildDeleteSQL(c.engine, ch.Table, keyCols)
		if err != nil {
			return err
		}
